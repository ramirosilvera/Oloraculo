...
export function LoginPage() {
  const { signInPassword, signUp, signInGoogle } = useAuth();
    ...
    <div className="h-full grid place-items-center px-4">
      <form onSubmit={submit} className="w-full max-w-sm rounded-2xl border border-ink-700 bg-ink-800/60 p-6 space-y-4">
        ...
        <div className="flex items-center gap-2 text-[10px] text-ink-600">
          <div className="flex-1 h-px bg-ink-700" /> o <div className="flex-1 h-px bg-ink-700" />
        </div>

        {/* Google */}
        <button type="button" disabled={busy}
          onClick={async () => { setMsg(null); const { error } = await signInGoogle(); if (error) setMsg({ text: error }); }}
          className="w-full flex items-center justify-center gap-2 rounded-lg border border-ink-600 bg-ink-900 hover:bg-ink-700 px-3 py-2 text-sm font-semibold text-gray-200 transition-colors disabled:opacity-50">
