...
export function useObjetivoDuracion(portfolioId: string | undefined) {
  ...
  useEffect(() => {
    let g = DEFAULT_MIN_GRADO_INVERSION_PCT, d = DEFAULT_MAX_DURACION_ANIOS;
    try {
    ...
    setMinGradoState(g); setMaxDuracionState(d);
  }, [key]);

  const persist = (g: number, d: number) => {
    if (!key) return;
    try { localStorage.setItem(key, JSON.stringify({ minGradoInversionPct: g, maxDuracionAnios: d })); } catch { /* */ }
  };
  return {
    minGradoInversionPct, maxDuracionAnios,
    setMinGradoInversionPct: (g: number) => { setMinGradoState(g); persist(g, maxDuracionAnios); },
    setMaxDuracionAnios: (d: number) => { setMaxDuracionState(d); persist(minGradoInversionPct, d); },
  };
}
