...
function CedearsPorSectorMetric({ viz }: { viz: DashboardViz }) {
  ...
  const mv: MetricValue = isLoading ? { status: 'loading' }
    ...
    : { status: 'ok', shape: 'categorico', format: 'usd-compact', items: resumen.porSector.map(s => ({ label: s.sector, value: s.pct * resumen.totalMkt })) };
  return <Render mv={mv} viz={viz} />;
}
...
function BonosDuracionPromedioMetric({ viz }: { viz: DashboardViz }) {
  ...
  const mv: MetricValue = isLoading ? { status: 'loading' }
    ...
    : { status: 'ok', shape: 'scalar', value: resumen.duracionPromedio, format: 'num', sub: 'años (Macaulay, ponderado)' };
  return <Render mv={mv} viz={viz} />;
}
...
function BonosGradoInversionMetric({ viz }: { viz: DashboardViz }) {
  ...
  const mv: MetricValue = isLoading ? { status: 'loading' }
    : !resumen ? { status: 'empty', motivo: 'Sin bonos en este portfolio.' }
    : { status: 'ok', shape: 'scalar', value: resumen.distribucionGrado.gradoInversion, format: 'pct' };
  return <Render mv={mv} viz={viz} />;
}

function BonosProximoCapitalMetric({ viz }: { viz: DashboardViz }) {
  const { active } = usePortfolios();
  const { bonos, isLoading } = useBonosCalc(active?.id);
  // Nota: acá NO se recalcula el calendario de capital (eso vive en CobrosResumen/DashboardPage y
  // usa amortizaciones_programadas) — esta tarjeta atómica solo confirma que hay bonos cargados y
  // remite al detalle, para no duplicar la lógica de capitalCalendar en un tercer lugar (ya está en
  // DashboardPage y en /cupones).
  const mv: MetricValue = isLoading ? { status: 'loading' }
    : bonos.length === 0 ? { status: 'empty', motivo: 'Sin bonos en este portfolio.' }
    : { status: 'empty', motivo: 'Ver el detalle exacto en la tarjeta Cobros o en /cupones.' };
  return <Render mv={mv} viz={viz} />;
}
...
function useRadarCompraAgresivaValue() {
  ...
  return { items, cikMap, cikLoading, riskFree, dcfMap, agresivos, onProbe, isLoading };
}
...
function RadarCompraAgresivaMetric({ viz }: { viz: DashboardViz }) {
  ...
  const mv: MetricValue = isLoading ? { status: 'loading' }
    : items.length === 0 ? { status: 'empty', motivo: 'Sin tickers en seguimiento — agregalos en /radar.' }
    : { status: 'ok', shape: 'scalar', value: agresivos.size, format: 'num', sub: `de ${items.length} en seguimiento` };
  return (
    <>
      {items.map(it => {
...
function MacroSemaforosMetric({ viz }: { viz: DashboardViz }) {
  ...
  const mv: MetricValue = isLoading ? { status: 'loading' }
    ...
    : {
        status: 'ok', shape: 'categorico', items: [
          { label: 'En verde', value: conteo.verdes, color: '#5FB49C' },
          { label: 'Atención', value: conteo.amarillos, color: '#E3B341' },
          { label: 'Estrés', value: conteo.rojos, color: '#D45F7A' },
        ].filter(i => i.value > 0),
      };
  return <Render mv={mv} viz={viz} />;
}
